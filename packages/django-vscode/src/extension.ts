		...
		window.withProgress(
			...
			async () => {
				...
				edits.forEach((change) => {
					const isTsOrJsFile =
						change.textDocument.uri.endsWith(".ts") || change.textDocument.uri.endsWith(".js")
					...
					change.edits.forEach((edit) => {
						if (isTsOrJsFile && !TsPlugin.isEnabled() && !edit.newText.endsWith(".svelte")) {
	...
	context.subscriptions.push(
		commands.registerTextEditorCommand("svelte.showCompiledCodeToSide", async (editor) => {
			...
			window.withProgress(
				...
				async () => {
					...
					return await window.showTextDocument(CompiledCodeContentProvider.jsPreviewWindowUri, {
						preview: true,
		...
		commands.registerTextEditorCommand("svelte.showCompiledCSSToSide", async (editor) => {
			...
			window.withProgress(
				...
				async () => {
					// Open a new preview window for the compiled code
					return await window.showTextDocument(CompiledCodeContentProvider.cssPreviewWindowUri, {
						preview: true,
						viewColumn: ViewColumn.Beside
					})
				}

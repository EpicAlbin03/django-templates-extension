...
describe("ConfigLoader", () => {
	...
	it("should load all config files below and the one inside/above given directory", async () => {
		...
		await assertFindsConfig(configLoader, "/some/path/comp.svelte", "/some/path/svelte.config.js")
	...
	it("will not load config multiple times if config loading started in parallel", async () => {
		...
		const configLoader = createConfigLoader(
			...
			{
				existsSync: (p) =>
					typeof p === "string" && p.endsWith(path.join("some", "path", "svelte.config.js"))
			},
			(module: URL) => {
				nrImportCalls++
	...
	it("can deal with missing config", () => {
		...
		assert.deepStrictEqual(configLoader.getConfig(normalizePath("/some/file.svelte")), undefined)
	})
	...
	it("prefers vite.config over svelte.config when both exist", async () => {
		...
		const configLoader = createConfigLoader(
			mockFdir(["svelte.config.js"]),
			{
				existsSync: (p) =>
					typeof p === "string" &&
					(p.endsWith(normalizePath("/some/path/svelte.config.js")) || p.endsWith(viteConfigPath))
			},
			(module: URL) => Promise.resolve({ default: { preprocess: module.toString() } }),
			process.features,
			loadFromVite
	...
	it("can scan svelte.config.ts", async () => {
		const configLoader = createConfigLoader(
			...
			{
				existsSync: (p) =>
					typeof p === "string" && p.endsWith(path.join("some", "path", "svelte.config.ts"))
			},
			(module: URL) => Promise.resolve({ default: { preprocess: module.toString() } }),
		...
		await assertFindsConfig(configLoader, "/some/path/comp.svelte", "/some/path/svelte.config.ts")
	})
	...
	it("can skips svelte.config.ts loading", async () => {
		...
		await assertFindsConfig(configLoader, "/some/path/comp.svelte", "/some/path/svelte.config.cjs")
	})
})

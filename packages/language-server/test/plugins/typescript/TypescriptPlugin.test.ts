...
describe("TypescriptPlugin", function () {
	...
	function setup(filename: string) {
		const docManager = new DocumentManager((args) =>
			args.uri.includes(".svelte") ? new Document(args.uri, harmonizeNewLines(args.text)) : document
		)
		...
		const lsAndTsDocResolver = new LSAndTSDocResolver(docManager, workspaceUris, lsConfigManager, {

...
describe("service", () => {
	...
	it("can find tsconfig and override with default config", async () => {
		...
		const ls = await getService(path.join(dirPath, "random.svelte"), rootUris, lsDocumentContext)
	...
	it("does not report no svelte files when loaded through import", async () => {
		...
		virtualSystem.writeFile(path.join(dirPath, "random.ts"), 'import {} from "./random.svelte"')
	...
	it("does not errors if referenced tsconfig matches no svelte files", async () => {
		...
		assert.equal(normalizePath(path.join(dirPath, "tsconfig_web.json")), lsContainer.tsconfigPath)
	...
	it("can watch tsconfig", async () => {
		...
		async function testAfterReload() {
			...
			assert.strictEqual(newLs.compilerOptions.strict, true, "expected to reload compilerOptions")

...
import { CancellationTokenSource, MarkupKind, Position, SignatureHelp } from "vscode-languageserver"
...
describe("SignatureHelpProvider", function () {
	...
	it("filter out svelte2tsx signature", async () => {
		...
		const result = await provider.getSignatureHelp(document, Position.create(18, 18), undefined)

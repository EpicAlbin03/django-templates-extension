...
describe("ts user preferences", function () {
	...
	it("provides auto import completion according to preferences", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(1, 14))
	...
	async function importCodeActionTest(filename: string, range: Range, context: CodeActionContext) {
		...
		const documentChange = codeAction[0].edit?.documentChanges?.[0] as TextDocumentEdit | undefined
	...
	it("provides auto import for svelte component when importModuleSpecifierEnding is js", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(4, 8))
	...
	it('provides auto import for context="module" export when importModuleSpecifierEnding is js', async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(1, 6))
	...
	it("provides import code action for svelte component when importModuleSpecifierEnding is js", async () => {
		...
		const codeAction = await codeActionProvider.getCodeActions(document, range, {
			diagnostics: [
				Diagnostic.create(range, "Cannot find name 'Imports'", DiagnosticSeverity.Error, 2304, "ts")
			]
		})
	...
	it("exclude auto import outside of the root", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(4, 7))

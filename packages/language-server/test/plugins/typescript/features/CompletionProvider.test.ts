...
describe("CompletionProviderImpl", function () {
	...
	it("provides completions", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(0, 49), {
	...
	it("provides completions on simple property access in mustache", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(5, 3), {
			triggerKind: CompletionTriggerKind.TriggerCharacter,
			triggerCharacter: "."
		})
	...
	it("provides event completions", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(5, 5), {
	...
	it("provide event completions for element from type definition", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(0, 14), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("provide tag name completion from type definition", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(0, 2), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("doesn't provide event completions inside attribute value", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(5, 17), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("provides event completions with correct text replacement span", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(5, 11), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("provides event completions from createEventDispatcher", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(6, 5), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("provides event completion for components with type definition", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(4, 16), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("provides event completion for components with type definition having multiple declarations of the same event", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(6, 16), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("does not provide completions inside style tag", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(4, 1), {
			triggerKind: CompletionTriggerKind.Invoked,
			triggerCharacter: "a"
		})
	...
	it("provides import completions in file with uppercase directory", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(1, 22), {
			triggerKind: CompletionTriggerKind.TriggerCharacter,
			triggerCharacter: "/"
		})
	...
	it("provides import completions for supported files", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(0, 27), {
			triggerKind: CompletionTriggerKind.TriggerCharacter,
			triggerCharacter: "/"
		})
	...
	it("resolve auto import completion (is first import in file)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(1, 3))
		document.version++
	...
	it("resolve auto import completion (is second import in file)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(2, 3))
		document.version++
	...
	it("resolve auto import completion (importing in same line as first import)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(0, 42))
		document.version++
	...
	it("resolve auto import completion (is second import, module-script present)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(7, 7))
		document.version++
	...
	it("resolve auto import completion in instance script (instance and module script present)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(5, 7))
		document.version++
	...
	it("resolve auto import completion in module script (instance and module script present)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(1, 7))
		document.version++
	...
	it("resolve auto import completion (importing a svelte component)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(2, 7))
		document.version++
	...
	it("resolve auto import completion (importing a svelte component, no script tag yet)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(0, 7))
		document.version++
	...
	it("resolve auto completion without auto import (a svelte component which was already imported)", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(3, 7))
		document.version++
	...
	it("doesnt suggest svelte auto import when already other import with same name present", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(2, 13))
		document.version++
	...
	it("resolve auto completion in correct place when already imported in module script", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(5, 8))
	...
	it("show jsDoc template completion", async () => {
		await testForJsDocTemplateCompletion(Position.create(1, 7), `/**${newLine} * $0${newLine} */`)
	})
	...
	it("handles completion in empty text attribute", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(4, 14))
		assert.deepStrictEqual(
			completions?.items.map((item) => item.label),
			["s", "m", "l"]
		)
	...
	it("can auto import package not in the program", async () => {
		...
		virtualSystem.writeFile(join(mockPackageDir, "index.d.ts"), "export function bar(): string")
	...
	it("can auto import new file", async () => {
		const virtualTestDir = getRandomVirtualDirPath(testFilesDir)
		const { document, lsAndTsDocResolver, lsConfigManager, docManager } = setupVirtualEnvironment({
			filename: "index.svelte",
			fileContent: "<script>b</script>",
			testDir: virtualTestDir
		})
	...
	it("provides import completions store that isnt imported yet", async () => {
		...
		assert.deepStrictEqual(detail, 'Add import from "./to-import"\n\nconst store: Writable<number>')
	})
	...
	it("provides namespace component completions for Svelte 5+", async () => {
		...
		const completions = await completionProvider.getCompletions(document, Position.create(4, 12), {
			triggerKind: CompletionTriggerKind.Invoked
		})
	...
	it("use correct position for component tag auto-import", async () => {
		...
		const itemAssumption = completionsAssumption?.items.find((item) => item.label === "Completions")
		assert.ok(
			itemAssumption,
			`expected this to exist. Otherwise, the "Completions" assertion is wrong`
		)

		const completions = await completionProvider.getCompletions(document, Position.create(1, 2))
		...
		assert.ok(!shouldNotExistItem, "expected auto-import be filtered out by current identifier")
	})
})

...
describe("RenameProvider", function () {
	...
	it("should rename and ignore generated", async () => {
		...
		const result = await provider.rename(renameDocIgnoreGenerated, Position.create(1, 8), "newName")
	...
	it("renames $props() binding in consumer", async () => {
		...
		const result = await provider.rename(renameRunesImporter, Position.create(7, 30), "newName")

...
namespace TagCloseRequest {
	export const type: RequestType<TextDocumentPositionParams, string | null, any> = new RequestType(
		"html/tag"
	)
}
...
export function startServer(options?: LSOptions) {
	...
	connection.onInitialize((evt) => {
		...
		Logger.setDebug(
			(evt.initializationOptions?.configuration?.svelte || evt.initializationOptions?.config)?.[
				"language-server"
			]?.debug
		)
		...
		if (evt.capabilities.textDocument?.diagnostic) {
			...
			connection.onRequest(
				...
				async (evt: DocumentDiagnosticParams, token): Promise<DocumentDiagnosticReport | null> => {
		...
		return {
			capabilities: {
				...
				documentHighlightProvider:
					evt.initializationOptions?.configuration?.svelte?.plugin?.svelte?.documentHighlight
						?.enable ?? true,

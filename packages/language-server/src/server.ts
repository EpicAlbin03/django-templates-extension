...
export function startServer(options?: LSOptions) {
	...
	connection.onInitialize((evt) => {
		...
		pluginHost.register(new HTMLPlugin(docManager, configManager, fileSystemProvider, workspaceFolders))
		pluginHost.register(
			new CSSPlugin(
				docManager,
				configManager,
		...
		if (evt.capabilities.textDocument?.diagnostic) {
			...
			connection.onRequest(
				...
				async (params: DocumentDiagnosticParams, token): Promise<DocumentDiagnosticReport | null> => {

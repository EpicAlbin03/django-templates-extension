		...
		FoldingRangeProvider
{
	...
	private async getCompletionsInternal(
		...
		if (isSASS(cssDocument)) {
			...
			return (
				doEmmetComplete(document, position, "sass", this.configManager.getEmmetConfig()) || null
			)
		}
	...
	private appendGlobalVars(items: CompletionItem[]): CompletionItem[] {
		...
		const additionalItems: CompletionItem[] = this.globalVars.getGlobalVars().map((globalVar) => ({

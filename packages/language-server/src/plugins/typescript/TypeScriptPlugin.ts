		...
		UpdateTsOrJsFile
{
	...
	constructor(
		...
		this.diagnosticsProvider = new DiagnosticsProviderImpl(this.lsAndTsDocResolver, configManager)
		...
		this.findFileReferencesProvider = new FindFileReferencesProviderImpl(this.lsAndTsDocResolver)
		...
		this.foldingRangeProvider = new FoldingRangeProviderImpl(this.lsAndTsDocResolver, configManager)
	...
	async getDocumentSymbols(
		...
		for (let symbol of symbols.slice(1)) {
			...
			if (
				...
				if (
					symbol.name === "props" &&
					document.getText().charAt(document.offsetAt(symbol.location.range.start)) !== "p"
				) {
			...
			if (start && end) {
				cb(
					SymbolInformation.create(
						...
						Range.create(tsDoc.positionAt(start.start), tsDoc.positionAt(end.start + end.length)),
						tsDoc.getURL(),
						container
					)
				)
		...
		const result = await Promise.all(
			defs.definitions.map(async (def) => {
				...
				if (isTextSpanInGeneratedCode(snapshot.getFullText(), def.textSpan)) {
					if (!is$storeVariableIn$storeDeclaration(snapshot.getFullText(), def.textSpan.start)) {
						return
					}
				...
				const defLocation = convertToLocationForReferenceOrDefinition(snapshot, def.textSpan)
	...
	async getSelectionRange(document: Document, position: Position): Promise<SelectionRange | null> {

...
export async function getService(
	...
	const nearestNodeModulesBoundary =
		lastNodeModulesIdx === 1 ? undefined : path.split("/").slice(0, lastNodeModulesIdx).join("/")
	...
	return getServiceForTsconfig(
		tsconfigPath,
		(nearestBoundary && urlToPath(nearestBoundary)) ?? docContext.tsSystem.getCurrentDirectory(),
		docContext
	)
...
async function createLanguageService(
	...
	const svelteCompiler =
		sveltePackageInfo.version.major >= 4 ? importSvelte(tsconfigPath || workspacePath) : undefined
	...
	const host: ts.LanguageServiceHost = {
		...
		getScriptFileNames,
		getScriptVersion: (fileName: string) => getSnapshotIfExists(fileName)?.version.toString() || "",
	...
	function createSnapshotManager(parsedCommandLine: ts.ParsedCommandLine, configFileName: string) {
	...
	function getScriptFileNames() {
		const projectFiles = languageServiceReducedMode ? [] : snapshotManager.getProjectFileNames()
	...
	function reduceLanguageServiceCapabilityIfFileSizeTooBig() {
		if (
			exceedsTotalSizeLimitForNonTsFiles(compilerOptions, tsconfigPath, snapshotManager, tsSystem)
		) {
		...
		if (totalNonTsFileSize > availableSpace) {
			...
			Logger.log(
				`Non TS file size exceeded limit (${totalNonTsFileSize}). ` +
					`Largest files: ${top5LargestFiles.map((file) => `${file.name}:${file.size}`).join(", ")}`
			)

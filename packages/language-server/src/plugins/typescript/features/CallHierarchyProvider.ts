...
export class CallHierarchyProviderImpl implements CallHierarchyProvider {
	...
	private async convertCallHierarchyItem(
		...
		const selectionRange = mapRangeToOriginal(snapshot, convertRange(snapshot, item.selectionSpan))
	...
	async getOutgoingCalls(
		...
		const result = await Promise.all(
			outgoingCalls.map(async (item): Promise<CallHierarchyOutgoingCall | null> => {
				if (item.to.name.startsWith("__sveltets") || item.to.containerName === "svelteHTML") {
					return null
				}
	...
	private redirectCallHierarchyItem(
		...
		if (!isSvelteFilePath(item.file) || !program || !(snapshot instanceof SvelteDocumentSnapshot)) {
			return null
		}
		...
		if (item.name === internalHelpers.renderName) {
			...
			const renderFunction = sourceFile.statements.find(
				(statement) => statement.getStart() <= item.selectionSpan.start && statement.getEnd() >= end
			)

...
export class CompletionsProviderImpl implements CompletionsProvider<CompletionResolveInfo> {
	...
	private getEventAndSlotLetCompletions(
		...
		return [
			...componentInfo
				.getEvents()
				.map((event) =>
					this.componentInfoToCompletionEntry(event, "on:", undefined, defaultTextEditRange)
				),
			...componentInfo
				.getSlotLets()
				.map((slot) =>
					this.componentInfoToCompletionEntry(slot, "let:", undefined, defaultTextEditRange)
				)
		]
	}
	...
	private getCustomElementCompletions(
		...
		let tags = getCustomElementsTags(lang, lsContainer, tsDoc).filter((t) => t.startsWith(tag.tag!))
	...
	private getCompletionListForDirectiveOrProps(
		...
		const props =
			...
			[]
		return CompletionList.create([...eventAndSlotLetCompletions, ...props], !!tsDoc.parserError)
	}
	...
	private toCompletionItem(
		...
		if (isCompletionInTag && !insertText && label[0] === '"' && label[label.length - 1] === '"') {
	...
	private getCompletionLabelAndInsert(snapshot: SvelteDocumentSnapshot, comp: ts.CompletionEntry) {
	...
	async resolveCompletion(
		...
		if (detail) {
			const { detail: itemDetail, documentation: itemDocumentation } = this.getCompletionDocument(
				tsDoc,
				detail,
				is$typeImport
			)

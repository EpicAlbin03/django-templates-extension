...
export class CodeLensProviderImpl implements CodeLensProvider {
	...
	async getCodeLens(document: Document): Promise<CodeLens[] | null> {
		...
		if (tsDoc.scriptKind === ts.ScriptKind.TS && clientTsConfig.implementationsCodeLens?.enabled) {
			collectors.push({
				type: "implementation",
				collect: (tsDoc, item, parent) =>
	...
	private anyCodeLensEnabled(lang: "typescript" | "javascript") {
		...
		return (
			vscodeTsConfig.referencesCodeLens?.enabled || vscodeTsConfig.implementationsCodeLens?.enabled
		)
	}

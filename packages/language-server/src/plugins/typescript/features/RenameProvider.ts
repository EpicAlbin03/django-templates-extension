...
export class RenameProviderImpl implements RenameProvider {
	...
	async rename(
		...
		return unique(
			convertedRenameLocations.filter((loc) => loc.range.start.line >= 0 && loc.range.end.line >= 0)
		).reduce(
			(acc, loc) => {
	...
	private async enhanceRenamesInCaseOf$Store(
		...
		for (const loc of renameLocations) {
			...
			if (isTextSpanInGeneratedCode(snapshot.getFullText(), loc.textSpan)) {
				if (isStoreVariableIn$storeDeclaration(snapshot.getFullText(), loc.textSpan.start)) {
					// User renamed store, also rename corresponding $store locations
					const storeRenameLocations = lang.findRenameLocations(
						snapshot.filePath,
						get$storeOffsetOf$storeDeclaration(snapshot.getFullText(), loc.textSpan.start),
						false,
						false,
						true
					)
					...
					const storeRenameLocations = lang.findRenameLocations(
						snapshot.filePath,
						getStoreOffsetOf$storeDeclaration(snapshot.getFullText(), loc.textSpan.start),
						false,
						false,
						true
					)
	...
	private async getAdditionLocationsForRenameOfPropInsideComponentWithProp(
		...
		const regex = new RegExp(
			`export\\s+let\\s+${this.getVariableAtPosition(tsDoc, lang, position)}($|\\s|;|:|\/\*|\/\/)`
		)
		...
		const idxOfOldPropName = tsDoc.getFullText().lastIndexOf(":", updatePropLocation.textSpan.start)
		...
		const replacementsForProp = (
			lang.findRenameLocations(updatePropLocation.fileName, idxOfOldPropName, false, false, true) ||
			[]
		).filter(
			(rename) =>
	...
	private async getAdditionalLocationsForRenameOfPropInsideOtherComponent(
		...
		if (
			getCanonicalFileName(updatePropLocation.fileName) === getCanonicalFileName(requestedFileName)
		) {
			return []
		}
	...
	private checkShortHandBindingOrSlotLetLocation(
		...
		return renameLocations.map((location) => {
			...
			if (!prefixText) {
				...
				if (original.startsWith(bind) && getNodeIfIsInHTMLStartTag(parent.html, rangeStart)) {
					return {
						...location,

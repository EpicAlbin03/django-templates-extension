...
export class InlayHintProviderImpl implements InlayHintProvider {
	...
	async getInlayHints(
		...
		if (cancellationToken?.isCancellationRequested || !this.areInlayHintsEnabled(userPreferences)) {
			return null
		}
	...
	private convertToTargetTextSpan(range: Range, snapshot: DocumentSnapshot) {
		...
		const end =
			generatedEndOffset.line < 0 ? snapshot.getLength() : snapshot.offsetAt(generatedEndOffset)
	...
	private async convertInlayHintLabelParts(inlayHint: ts.InlayHint, snapshotMap: SnapshotMap) {
		...
		const convertPromises = inlayHint.displayParts.map(
			async (part): Promise<InlayHintLabelPart> => {
				...
				const originalLocation = mapLocationToOriginal(snapshot, convertRange(snapshot, part.span))
	...
	private isGeneratedVariableTypeHint(sourceFile: ts.SourceFile, inlayHint: ts.InlayHint): boolean {
		if (inlayHint.kind !== ts.InlayHintKind.Type) {
			return false
		}

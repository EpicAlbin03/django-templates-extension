...
export class CodeActionsProviderImpl implements CodeActionsProvider {
	 ...
	 */
	private removeDuplicatedComponentImport(insertedNames: Set<string>, change: ts.FileTextChanges) {
				...
				return TextDocumentEdit.create(
					...
					change.textChanges
						.map((edit) => {
							...
							return this.fixIndentationOfImports(TextEdit.replace(range, edit.newText), document)
						})
						.filter(
							(edit) =>
				...
				change.textChanges
					.map((edit) => {
						if (fix.fixName === FIX_IMPORT_FIX_NAME && snapshot instanceof SvelteDocumentSnapshot) {
							...
							const startPos =
								namePosition ??
								this.findDiagnosticForImportFix(document, edit, getDiagnostics())?.range?.start ??
								Position.create(0, 0)
	...
	private getFixAllActions(
		...
		for (const codeFix of codeFixes) {
			...
			const codeAction = CodeAction.create(codeFix.fixAllDescription, CodeActionKind.QuickFix)
	...
	private getComponentImportQuickFix(
		...
		for (const [name, position] of nameToPosition) {
			...
			const entries = completion?.entries
				...
				.sort(
					(a, b) =>
						this.numberOfDirectorySeparators(ts.displayPartsToString(a?.sourceDisplay ?? [])) -
						this.numberOfDirectorySeparators(ts.displayPartsToString(b?.sourceDisplay ?? []))
				)
				.filter(isNotNullOrUndefined)
	...
	private checkEndOfFileCodeInsert(
		...
		if (resultRange.start.line < 0 || resultRange.end.line < 0) {
			if (
				...
				return Range.create(document.moduleScriptInfo.endPos, document.moduleScriptInfo.endPos)
			}
	...
	private checkTsNoCheckCodeInsert(document: Document, edit: ts.TextChange): TextEdit | undefined {
	...
	private fixPropsCodeActionRange(start: Position, document: Document): Position | undefined {
		...
		if ((charBeforeExport !== undefined && !charBeforeExport.trim()) || charBeforeExport === ";") {

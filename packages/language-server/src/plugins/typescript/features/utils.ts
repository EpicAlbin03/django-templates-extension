...
>(
	(node) => ts.isFunctionDeclaration(node) && node?.name?.getText() === internalHelpers.renderName,
	ts.isSourceFile
)

const isRenderFunctionBody = nodeAndParentsSatisfyRespectivePredicates(ts.isBlock, isRenderFunction)

			...
			for (const item of projectItems) {
				...
				if (!seen) {
					symbols.set(item.name, [
						[item, this.mapWorkspaceSymbol(item, await snapshots.retrieve(item.fileName))]
					])
					continue
				}
				...
				seen.push([item, this.mapWorkspaceSymbol(item, await snapshots.retrieve(item.fileName))])
			}
		}

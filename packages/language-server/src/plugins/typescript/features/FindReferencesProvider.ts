...
export class FindReferencesProviderImpl implements FindReferencesProvider {
	...
	async findReferences(
		...
		references.push(
			...(await this.getStoreReferences(references, tsDoc, snapshots, lang, cancellationToken))
		)

		const locations = await Promise.all(
			references.map(async (ref) => this.mapReference(ref, context, snapshots, cancellationToken))
		)
	...
	private async getStoreReferences(
		...
		if (storeReference) {
			const additionalReferences =
				lang.findReferences(
					tsDoc.filePath,
					getStoreOffsetOf$storeDeclaration(tsDoc.getFullText(), storeReference.textSpan.start)
				) || []
	...
	private async checkIfHasAliasedComponentReference(
		...
		const references = await Promise.all(
			nonAliasDefinitions.map((definition) =>
				this.componentReferencesProvider.findComponentReferences(pathToUrl(definition.fileName))
			)
		)

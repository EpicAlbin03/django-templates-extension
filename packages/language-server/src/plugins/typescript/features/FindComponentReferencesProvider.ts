...
import { convertToLocationRange, hasNonZeroRange, offsetOfGeneratedComponentExport } from "../utils"
...
export class FindComponentReferencesProviderImpl implements FindComponentReferencesProvider {
	...
	async findComponentReferences(uri: string): Promise<Location[] | null> {
		...
		const references = lang.findReferences(tsDoc.filePath, offsetOfGeneratedComponentExport(tsDoc))
		if (!references) {
			return null
		}

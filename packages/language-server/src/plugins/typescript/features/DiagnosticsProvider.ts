...
export function mapAndFilterDiagnostics(
	...
	const source =
		tsDoc.scriptKind === ts.ScriptKind.TSX || tsDoc.scriptKind === ts.ScriptKind.TS ? "ts" : "js"
...
function rangeMapper(
	...
	const get$$PropsDefWithCache = memoize(() => (lang ? get$$PropsDef(lang, snapshot) : undefined))

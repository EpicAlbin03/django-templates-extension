...
export class PluginHost implements LSProvider, OnWatchFileChanges {
  ...
  async getDiagnosticsForPullMode(
    ...
    const unchanged = results.filter(([, res]) => res.kind === "unchanged").map(([name]) => name);
  ...
  async doHover(textDocument: TextDocumentIdentifier, position: Position): Promise<Hover | null> {
    ...
    return this.execute<Hover>("doHover", [document, position], ExecuteMode.FirstNonNull, "high");
  }
  ...
  async getHierarchicalDocumentSymbols(
    ...
    const symbols = flat
      .map((s) =>
        DocumentSymbol.create(s.name, undefined, s.kind, s.location.range, s.location.range, []),
      )
      .sort((a, b) => {
  ...
  private async execute<T>(
    ...
    if (
      ...
      (this.requestTimings[name]?.[0] > 500 ||
        Object.values(this.requestTimings).filter((t) => t[0] > 400 && now - t[1] < 60 * 1000)
          .length > 2)
    ) {
      ...
      const debounce = async (): Promise<boolean> => {
        ...
        this.deferredRequests[name] = [
          ...
          new Promise<void>((resolve, reject) => {
            setTimeout(() => {
              if (!this.deferredRequests[name] || this.deferredRequests[name][0] === id) {
                resolve();
              } else {

...
function printStatement(node: StatementNode): Doc {
    ...
    if (['else', 'elif', 'empty', 'plural'].includes(node.keyword) && block && !block.inTag && !block.inAttribute) {
        ...
        const mapped = await Promise.all(
            segments.map(async (segment) => {
                ...
                return utils.mapDoc(doc, (currentDoc) => {
                    ...
                    return replacePlaceholdersInString(currentDoc, ids, (id) => path.call(print, 'nodes', id));
                });
            }),
        );
